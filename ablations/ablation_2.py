...
"""
Dual‑Branch RCAN ×4  (Ablation #2)
  - Shallow + Deep branches (6 & 10 residual groups, respectively).
  - NO Channel / Spatial attention (plain RCAB blocks).
Everything else matches the baseline Dual‑Branch‑with‑CBAM implementation.
"""
import os
import math
import random
...
class Config:
    def __init__(self):
        ...
        # Model capacity (same as baseline)
        ...
        # Book‑keeping
        self.save_dir = f"DualBranch_RCAN_noCBAM_x{self.scale_factor}"
        self.save_freq = 100
        self.val_freq = 1
...
# ────────────────── Plain RCAB & Residual Group (no attention) ────────────


class RCABlock(nn.Module):
    """Residual block without attention (Conv‑ReLU‑Conv)."""
...
class ResidualGroup(nn.Module):
    def __init__(self, n_feats, n_blocks):
        super().__init__()
        modules = [RCABlock(n_feats) for _ in range(n_blocks)]
...
# ──────────────────────── Dual‑Branch RCAN (no CBAM) ──────────────────────


class DualBranchRCAN_NoCBAM(nn.Module):
    def __init__(self,
                 scale_factor=4,
                 in_channels=3,
                 n_feats=128,
                 num_groups_shallow=6,
                 num_groups_deep=10,
        ...
        self.shallow_groups = nn.ModuleList(
            [ResidualGroup(n_feats, num_blocks_per_group)
    ...
    def forward(self, x):
        ...
        # Fusion & upsample
        out = self.fuse(torch.cat([s, d], dim=1))
...
def train():
    ...
    model = DualBranchRCAN_NoCBAM(scale_factor=cfg.scale_factor,
