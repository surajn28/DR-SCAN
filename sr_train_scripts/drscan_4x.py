...
class Config:
    def __init__(self):
        ...
        self.lr_milestones = [1000, 2000, 3000]
        self.lr_gamma = 0.7
        ...
        self.resume_checkpoint = f"DualBranch_x4/checkpoints/epoch_1900.pth"
...
def rgb_to_y(img: torch.Tensor) -> torch.Tensor:
    ...
    Returns: Tensor, shape (..., 1, H, W), the Y channel via BT.601
    """
...
def crop_border(x: torch.Tensor, border: int) -> torch.Tensor:
    """
    Crops border pixels on each side.  No crop if image smaller than 2*border.
    """
    if border == 0 or x.size(-1) <= 2*border or x.size(-2) <= 2*border:
        return x
    return x[..., border:-border, border:-border]


def calculate_psnr(sr_y, hr_y, scale):
    """
    sr_y, hr_y : (..., 1, H, W) luminance tensors in [0,1]
    scale      : int, crop this many pixels on each border before PSNR
    """
    sr_c = crop_border(sr_y, scale)
    hr_c = crop_border(hr_y, scale)

    sr_np = np.clip(sr_c.detach().cpu().numpy(), 0.0, 1.0)
    hr_np = np.clip(hr_c.detach().cpu().numpy(), 0.0, 1.0)
...
def validate(model, loader, pix_loss, perc_loss, cfg):
    ...
    for lr, hr in loader:
        ...
        loss = pix_loss(sr, hr) + \
            cfg.perceptual_weight * perc_loss(sr, hr)

        tot_loss += loss.item()
        tot_psnr += calculate_psnr(sr_y, hr_y, cfg.scale_factor)
        n += 1
...
def train(cfg):
    ...
    # scaler = GradScaler() # mixed-precision  did not work really well hence using FP32

    # sch = MultiStepLR(opt, milestones=cfg.lr_milestones, gamma=cfg.lr_gamma)
    sch = CosineAnnealingLR(opt, T_max=cfg.num_epochs, eta_min=1e-6)
    ...
    for epoch in range(start_epoch, cfg.num_epochs + 1):
        ...
        for lr, hr in train_loader:
            ...
            ep_loss += loss.item()
            ep_psnr += calculate_psnr(sr_y, hr_y, cfg.scale_factor)
            steps += 1
